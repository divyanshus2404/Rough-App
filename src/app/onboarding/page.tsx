...
import { completeVerification } from '@/app/actions/auth'
...
export default function OnboardingPage() {
  ...
  const handleNext = () => {
    setError('')
    if (step === 1 && !enrollmentNumber) {
      setError('Please enter your Enrollment Number.')
      return
    }
    if (step === 2 && !faceScanBase64) {
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="text-center mb-8">
                ...
                <p className="mt-2 text-sm text-muted">Provide your unique enrollment number to continue.</p>
              </div>
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <button onClick={() => setStep(2)} disabled={isSubmitting} className="w-full mt-3 py-3 text-muted font-bold hover:text-foreground transition-all">Back</button>
            </motion.div>
          )}
